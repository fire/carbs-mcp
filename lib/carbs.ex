defmodule Carbs do
      ...
      {:ok, serialized_state} ->
        ...
        optimizer_attrs = %{
          ...
          better_direction_sign:
            config.better_direction_sign || Params.default().better_direction_sign,
          ...
          is_wandb_logging_enabled:
            config.is_wandb_logging_enabled || Params.default().is_wandb_logging_enabled,
          ...
          is_saved_on_every_observation:
            config.is_saved_on_every_observation || Params.default().is_saved_on_every_observation,
          initial_search_radius:
            config.initial_search_radius || Params.default().initial_search_radius,
          exploration_bias: config.exploration_bias || Params.default().exploration_bias,
          num_candidates_for_suggestion_per_dim:
            config.num_candidates_for_suggestion_per_dim ||
              Params.default().num_candidates_for_suggestion_per_dim,
          ...
          min_pareto_cost_fraction:
            config.min_pareto_cost_fraction || Params.default().min_pareto_cost_fraction,
          is_pareto_group_selection_conservative:
            config.is_pareto_group_selection_conservative ||
              Params.default().is_pareto_group_selection_conservative,
          is_expected_improvement_pareto_value_clamped:
            config.is_expected_improvement_pareto_value_clamped ||
              Params.default().is_expected_improvement_pareto_value_clamped,
          is_expected_improvement_value_always_max:
            config.is_expected_improvement_value_always_max ||
              Params.default().is_expected_improvement_value_always_max,
          outstanding_suggestion_estimator:
            config.outstanding_suggestion_estimator ||
              Params.default().outstanding_suggestion_estimator,
          num_observations: 0
        }
  ...
  """
  def load(name) do
    case Repo.get_by(Optimizer, name: name)
         |> Repo.preload([:params, :observations, :suggestions]) do
      nil ->
        {:error, :not_found}
      ...
      optimizer ->
        ...
        config = %Params{
          ...
          is_pareto_group_selection_conservative:
            optimizer.is_pareto_group_selection_conservative,
          is_expected_improvement_pareto_value_clamped:
            optimizer.is_expected_improvement_pareto_value_clamped,
          is_expected_improvement_value_always_max:
            optimizer.is_expected_improvement_value_always_max,
            ...
            observations =
              ...
              |> Enum.map(fn obs ->
                ...
                Observation.new(
                  ...
                  obs.output,
                  obs.cost || 1.0,
                  obs.is_failure || false
                )
              end)

            # Replay observations
            final_state =
              Enum.reduce(observations, serialized_state, fn obs, state ->
  ...
  defp convert_value(%{__struct__: module} = struct)
       when module in [Params, Param, Observation] do
    struct
    |> Map.from_struct()
    |> Enum.map(fn {k, v} -> {k, convert_value(v)} end)
    |> Enum.into(%{})
